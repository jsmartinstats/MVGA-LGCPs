function covmatoutput = covfun_geoanisot_quick(v1,v2,covtype,sig,alp,gam,the,ome,zet)
...
%
% This function performs the same operations as covfun_geoanisot.m, but is quicker.
% 
% 
% INPUT
...
%%
    ...
    n = length(v1);
    P = size(sig,1);
    rmat = repmat((1./ome),n,1)...
        .*sqrt( repmat((c2+invzetsq.*s2),n,1).*reshape(repmat(v1'.^2,P,P),P*n,P) ...
        + 2.*repmat((1-invzetsq).*sc,n,1).*reshape(repmat(v1'.*v2',P,P),P*n,P) ...
        + repmat((s2+invzetsq.*c2),n,1).*reshape(repmat(v2'.^2,P,P),P*n,P) );
    sig = repmat(sig,n,1);
    alp = repmat(alp,n,1);
    gam = repmat(gam,n,1);
    if strcmp(covtype,'powexp')
        covmatoutput = sig .* exp(-(rmat./alp).^gam);
